...
BOOST_AUTO_TEST_CASE(ISNPS)
{
	...
	typedef otl::optimizer::isnps::ISNPS<_TReal, _TDecision, _TRandom &> _TOptimizer;
	...
	const std::vector<_TReal> objectiveLower(nObjectives, 0);
	const boost::numeric::ublas::vector<_TReal> convergenceDirection(nObjectives, 1);
	const _TReal angle1 = 2.4 * boost::math::constants::pi<_TReal>() / 180;
	const _TReal angle2 = 45 * boost::math::constants::pi<_TReal>() / 180;
	const _TReal amplification = 3;
	_TOptimizer optimizer(random, problem, initial, crossover, mutation, objectiveLower, convergenceDirection, angle1, angle2, amplification);
