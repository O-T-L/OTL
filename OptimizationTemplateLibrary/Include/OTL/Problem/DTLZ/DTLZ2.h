...
class DTLZ2 : public DTLZ<_TReal>
{
public:
	...
	DTLZ2(const size_t nObjectives, const size_t distDecisions = 10);
	~DTLZ2(void);
...
template <typename _TReal>
DTLZ2<_TReal>::DTLZ2(const size_t nObjectives, const size_t distDecisions)
	: TSuper(nObjectives, TBoundary(nObjectives - 1 + distDecisions, std::make_pair<TReal, TReal>(0, 1)))
{
}

template <typename _TReal>
DTLZ2<_TReal>::~DTLZ2(void)
{
}

template <typename _TReal>
size_t DTLZ2<_TReal>::_DoEvaluate(TSolution &solution)
{
...
template <typename _TReal>
void DTLZ2<_TReal>::_Evaluate(const TDecision &decision, std::vector<TReal> &objective)
{
