...
template <typename _TReal>
ZDT2<_TReal>::ZDT2(const size_t nDistDecisions)
	: TSuper(TBoundary(1 + nDistDecisions, std::make_pair<TReal, TReal>(0, 1)))
{
}

template <typename _TReal>
ZDT2<_TReal>::~ZDT2(void)
{
}

template <typename _TReal>
size_t ZDT2<_TReal>::_DoEvaluate(TSolution &solution)
{
...
template <typename _TReal>
void ZDT2<_TReal>::_Evaluate(const TDecision &decision, std::vector<TReal> &objective)
{
