...
class CF : public Problem<_TReal, std::vector<_TReal> >, public otl::utility::WithSpaceBoundary<_TReal>
{
public:
	...
	CF(const size_t nObjectives, const TBoundary &boundary);
	~CF(void);
...
template <typename _TReal>
CF<_TReal>::CF(const size_t nObjectives, const TBoundary &boundary)
	: TSuper(nObjectives)
...
template <typename _TReal>
void CF<_TReal>::_DoFix(std::vector<TReal> &objective)
{
}
}
}
