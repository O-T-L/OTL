...
public:
	...
	UF10(const size_t nDistDecisions = 28);
	~UF10(void);
...
template <typename _TReal>
UF10<_TReal>::UF10(const size_t nDistDecisions)
	: TSuper(3, GenerateDecisionSpace(2 + nDistDecisions))
{
}

template <typename _TReal>
UF10<_TReal>::~UF10(void)
{
}

template <typename _TReal>
typename UF10<_TReal>::TBoundary UF10<_TReal>::GenerateDecisionSpace(const size_t dimension)
{
...
template <typename _TReal>
size_t UF10<_TReal>::_DoEvaluate(TSolution &solution)
{
...
template <typename _TReal>
void UF10<_TReal>::_Evaluate(const TDecision &decision, std::vector<TReal> &objective)
{
	...
	objective[0] = cos(alpha) * cos(beta) + 2 * distance::Distance9(decision, 3);
	objective[1] = cos(alpha) * sin(beta) + 2 * distance::Distance9(decision, 4);
	objective[2] = sin(alpha) + 2 * distance::Distance9(decision, 2);
}
}
}
}
