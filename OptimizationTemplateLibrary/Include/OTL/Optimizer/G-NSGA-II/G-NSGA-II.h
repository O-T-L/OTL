...
class G_NSGA_II : public Metaheuristic<std::vector<otl::optimizer::nsga_ii::Individual<_TReal, _TDecision> > >, public otl::utility::WithRandom<_TRandom>, public otl::crossover::WithCrossover<_TReal, _TDecision>, public otl::mutation::WithMutation<_TReal, _TDecision>
{
public:
	...
	G_NSGA_II(TRandom random, TProblem &problem, const std::vector<TDecision> &initial, TCrossover &crossover, TMutation &mutation, const std::vector<TReal> &referencePoint);
	~G_NSGA_II(void);
...
template <typename _TReal, typename _TDecision, typename _TRandom>
G_NSGA_II<_TReal, _TDecision, _TRandom>::G_NSGA_II(TRandom random, TProblem &problem, const std::vector<TDecision> &initial, TCrossover &crossover, TMutation &mutation, const std::vector<TReal> &referencePoint)
	: TSuper(problem)
...
template <typename _TReal, typename _TDecision, typename _TRandom>
G_NSGA_II<_TReal, _TDecision, _TRandom>::~G_NSGA_II(void)
{
}

template <typename _TReal, typename _TDecision, typename _TRandom>
const std::vector<_TReal> &G_NSGA_II<_TReal, _TDecision, _TRandom>::GetReferencePoint(void) const
{
	return referencePoint_;
}

template <typename _TReal, typename _TDecision, typename _TRandom>
typename G_NSGA_II<_TReal, _TDecision, _TRandom>::TSolutionSet G_NSGA_II<_TReal, _TDecision, _TRandom>::MakeOffspring(const TSolutionSet &ancestor)
{
...
template <typename _TReal, typename _TDecision, typename _TRandom>
bool G_NSGA_II<_TReal, _TDecision, _TRandom>::Dominate(const TIndividual &individual1, const TIndividual &individual2) const
{
	return G_Dominance(individual1.objective_, individual2.objective_, referencePoint_);
}

template <typename _TReal, typename _TDecision, typename _TRandom>
void G_NSGA_II<_TReal, _TDecision, _TRandom>::_DoStep(void)
{
...
template <typename _TReal, typename _TDecision, typename _TRandom>
template <typename _TPointer, typename _TIterator> _TIterator G_NSGA_II<_TReal, _TDecision, _TRandom>::_SelectNoncritical(const std::list<_TPointer> &front, _TIterator begin, _TIterator end)
{
...
template <typename _TReal, typename _TDecision, typename _TRandom>
template <typename _TPointer, typename _TIterator> _TIterator G_NSGA_II<_TReal, _TDecision, _TRandom>::_SelectCritical(const std::list<_TPointer> &front, _TIterator begin, _TIterator end)
{
	std::vector<_TPointer> _front(front.begin(), front.end());
	otl::optimizer::nsga_ii::CrowdingDistanceAssignment<TReal>(_front.begin(), _front.end());
...
template <typename _TReal, typename _TDecision, typename _TRandom>
const typename G_NSGA_II<_TReal, _TDecision, _TRandom>::TIndividual *G_NSGA_II<_TReal, _TDecision, _TRandom>::_Compete(const std::vector<const TIndividual *> &competition) const
{
