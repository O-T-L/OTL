...
@Conference{,
  ...
  Author                   = {Raquel Hernández Gómez and Carlos A. {Coello Coello}},
...
class MonteCarloSMS_EMOA : public Metaheuristic<std::vector<Individual<_TReal, _TDecision> > >, public otl::utility::WithRandom<_TRandom>
{
public:
...
template <typename _TReal, typename _TDecision, typename _TRandom>
size_t MonteCarloSMS_EMOA<_TReal, _TDecision, _TRandom>::GetSampleSize(void) const
{
	return nSample_;
}

template <typename _TReal, typename _TDecision, typename _TRandom>
std::vector<const typename MonteCarloSMS_EMOA<_TReal, _TDecision, _TRandom>::TIndividual *> MonteCarloSMS_EMOA<_TReal, _TDecision, _TRandom>::MatingSelection(const size_t offspringSize, const TSolutionSet &ancestor)
{
	return otl::optimizer::nsga_ii::MatingSelection(offspringSize, ancestor.begin(), ancestor.end(), this->GetRandom(), &_Compete);
}

template <typename _TReal, typename _TDecision, typename _TRandom>
bool MonteCarloSMS_EMOA<_TReal, _TDecision, _TRandom>::Dominate(const TIndividual &individual1, const TIndividual &individual2)
{
	return otl::utility::relation::Dominate(individual1.objective_, individual2.objective_);
}

template <typename _TReal, typename _TDecision, typename _TRandom>
bool MonteCarloSMS_EMOA<_TReal, _TDecision, _TRandom>::_Dominate(const TIndividual *individual1, const TIndividual *individual2)
{
	return Dominate(*individual1, *individual2);
}

template <typename _TReal, typename _TDecision, typename _TRandom>
const typename MonteCarloSMS_EMOA<_TReal, _TDecision, _TRandom>::TIndividual *MonteCarloSMS_EMOA<_TReal, _TDecision, _TRandom>::_Compete(const std::vector<const TIndividual *> &competition)
{
...
template <typename _TReal, typename _TDecision, typename _TRandom>
void MonteCarloSMS_EMOA<_TReal, _TDecision, _TRandom>::_Reduce(TSolutionSet &population, TSolutionSet &solutionSet)
{
