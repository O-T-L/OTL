...
public:
	...
	UF2(const size_t nDistDecisions = 29);
	~UF2(void);
...
template <typename _TReal>
UF2<_TReal>::UF2(const size_t nDistDecisions)
...
template <typename _TReal>
typename UF2<_TReal>::TBoundary UF2<_TReal>::GenerateDecisionSpace(const size_t dimension)
{
...
template <typename _TReal>
size_t UF2<_TReal>::_DoEvaluate(TSolution &solution)
{
...
void UF2<_TReal>::_Evaluate(const TDecision &decision, std::vector<TReal> &objective)
{
	...
	objective[1] = 1 - sqrt(x) + 2 * distance::Distance2_1(decision, 1);
}
}
}
}
