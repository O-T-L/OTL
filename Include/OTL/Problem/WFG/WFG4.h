...
class WFG4 : public StdWFG<_TReal>
{
public:
	...
	WFG4(const size_t nObjectives, const size_t posGroups = 2, const size_t distDecisions = 20);
	~WFG4(void);
...
template <typename _TReal>
WFG4<_TReal>::WFG4(const size_t nObjectives, const size_t posGroups, const size_t distDecisions)
	: TSuper(nObjectives, posGroups, distDecisions)
{
}

template <typename _TReal>
WFG4<_TReal>::~WFG4(void)
{
}

template <typename _TReal>
void WFG4<_TReal>::Transition1(TDecision &decision)
{
	for(size_t i = 0; i < decision.size(); ++i)
		decision[i] = transform::shift::MultiModal<TReal>(decision[i], 30, 10, 0.35);
}

template <typename _TReal>
_TReal WFG4<_TReal>::Transition2(const TDecision &decision, const size_t nPosDecisions, TDecision &posDecision)
{
	return WFG2<TReal>::Transition3(decision, nPosDecisions, posDecision);
}

template <typename _TReal>
void WFG4<_TReal>::Shape(const TDecision &decision, const _TReal distance, std::vector<TReal> &objective)
{
...
template <typename _TReal>
size_t WFG4<_TReal>::_DoEvaluate(TSolution &solution)
{
...
template <typename _TReal>
void WFG4<_TReal>::_Evaluate(const TDecision &decision, std::vector<TReal> &objective)
{
