...
class DTLZ5 : public DTLZ<_TReal>
{
public:
	...
	DTLZ5(const size_t nObjectives, const size_t distDecisions = 10);
	~DTLZ5(void);
...
template <typename _TReal>
DTLZ5<_TReal>::DTLZ5(const size_t nObjectives, const size_t distDecisions)
	: TSuper(nObjectives, TBoundary(nObjectives - 1 + distDecisions, std::make_pair<TReal, TReal>(0, 1)))
{
}

template <typename _TReal>
DTLZ5<_TReal>::~DTLZ5(void)
{
}

template <typename _TReal>
size_t DTLZ5<_TReal>::_DoEvaluate(TSolution &solution)
{
